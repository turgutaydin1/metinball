(()=>{
  ...
  const BACK_URLS={
    turgut:'./assets/m3_turgut_back.svg?v=0222',
    zeko:'./assets/m3_zeko_back.svg?v=0222',
    nafi:'./assets/m3_nafi_back.svg?v=0222',
    baki:'./assets/m3_baki_back.svg?v=0222'
  };
  ...
  const previousSetPlayerPose=GameScene.prototype.setPlayerPose;
  const previousPlayLiveRun=GameScene.prototype.playLiveRun;
  const previousUpdateMission3=GameScene.prototype.updateMission3;

  // Görev 3'te ana oyunun önden koşu/idle karelerine geri dönmesini engelle.
  GameScene.prototype.setPlayerPose=function(pose='idle'){
    if(this.selectedMission===3 && this.m3BackLocked){
      try{this.playerImage?.stop?.()}catch{}
      return;
    }
    return previousSetPlayerPose.apply(this,arguments);
  };

  GameScene.prototype.playLiveRun=function(){
    if(this.selectedMission===3 && this.m3BackLocked){
      try{this.playerImage?.stop?.()}catch{}
      return true;
    }
    return previousPlayLiveRun.apply(this,arguments);
  };
  ...
  GameScene.prototype.updateMission3=function(dt){
    const out=previousUpdateMission3.apply(this,arguments);
    if(this.m3BackLocked && this.m3BackKey && this.playerImage?.active){
      try{
        if(this.playerImage.texture?.key!==this.m3BackKey){
          this.playerImage.stop?.();
          this.playerImage.setTexture(this.m3BackKey);
        }
        const targetH=this.m3BackTargetH||Math.min(355,this.scale.height*.395);
        const targetW=this.m3BackTargetW||targetH*(280/520);
        if(Math.abs((this.playerImage.displayHeight||0)-targetH)>2 || Math.abs((this.playerImage.displayWidth||0)-targetW)>2){
          this.playerImage.setDisplaySize(targetW,targetH);
        }
        if(Math.abs((this.playerImage.y||0)+targetH/2)>3){
          this.playerImage.setY(-targetH/2);
        }
        this.playerImage.setVisible(true).setAlpha(1);
      }catch{}
    }
    return out;
  };
